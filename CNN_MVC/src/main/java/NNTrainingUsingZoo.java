...
public class NNTrainingUsingZoo {
	...
	public static void main(String[] args) throws IOException {
		...
		InputSplit trainData = new FileSplit(citoTrainRootDir, BaseImageLoader.ALLOWED_FORMATS, new Random());
		InputSplit testData = new FileSplit(citoTestRootDir, BaseImageLoader.ALLOWED_FORMATS, new Random());
		...
		DataSetIterator citoTrainDataSet = new RecordReaderDataSetIterator(trainReader, MINI_BATCH_SIZE, 1, N_LABELS);
		DataSetIterator citoTestDataSet = new RecordReaderDataSetIterator(testReader, MINI_BATCH_SIZE, 1, N_LABELS);
		...
		for (int i = 0; i < numEpochs; i++) {
			log.info("Epoch " + i);
			modelTransfer.fit(citoTrainDataSet);
		}
		...
		for (int i = 0; i < transformedDataEpochs; i++) {
			...
			for (int j = 0; j < transforms.length; j++) {
				...
				citoTrainDataSet = new RecordReaderDataSetIterator(trainReader, MINI_BATCH_SIZE, 1, N_LABELS);
				modelTransfer.fit(citoTrainDataSet);
			}
		}
		...
		while (citoTestDataSet.hasNext()) {
			DataSet next = citoTestDataSet.next();
